    ...
    def check_passw(self):
        ...
        if len(text) > 0:
            ...
            if text == reader[0]:
                ...
                self.close()
                QMessageBox.information(self, '', 'Добро пожаловать!') # type: ignore
            else:
                QMessageBox.information(self, 'Внимание', 'Пароль введён неверно!') # type: ignore
        else:
            QMessageBox.information(self, 'Внимание', 'Введите пароль!') # type: ignore
        ...
        while True:
            for k, v in self.dictIP.items():
                if v[2] == 2:
                    ...
                    if CheckEthernet.ethernet() == False:
                        response = 1
                    self.msleep(1)
...
# Метки
class Label(QLabel): # type: ignore
    clicked = QtCore.pyqtSignal(str, str)

    def __init__(
        self,
        background = QColor("white"), # type: ignore
        parent = None
    ):
        super().__init__(parent)
...
class MyWin(QtWidgets.QMainWindow):
    def __init__(self, parent=None):
        ...
        self.ui.tableWidget.horizontalHeader().setSectionResizeMode(QHeaderView.ResizeToContents) # type: ignore
        self.ui.tableWidget.horizontalHeader().setMinimumSectionSize(0)
        self.ui.tableWidget.verticalHeader().setSectionResizeMode(QHeaderView.ResizeToContents) # type: ignore
    ...
    def coordinate(self, response, iP):
        ...
        if not _lbl:
            ...
            records = connect_db("""SELECT Yi, Gor, Location FROM ips WHERE INET_NTOA(ip) = '{}'""".format(iP), 'all')   
            ...
            if response == 1:
                if iP:
                    # Красный
                    lab.background = QColor("#fa7f72") # type: ignore
                else:
                    # Оранжевый
                    lab.background = QColor("#ffaa00") # type: ignore
            else:
                # Зелёный
                lab.background = QColor("#54e346") # type: ignore
    ...
    def on_threadSignal_1(self, response_1, k_1, v_1):
        ...
        if response_1:
            ...
            records = connect_db("""SELECT * FROM ips WHERE INET_NTOA(ip) = '{}'""".format(k_1), 'all')
...
if __name__ == '__main__':
    ...
    if CheckEthernet.ethernet() == False:
        QMessageBox.information(myapp , 'Внимание', 'Нет подключения к интернету!') # type: ignore
